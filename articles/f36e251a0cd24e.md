---
title: "Docker一強時代が終わる前に押さえておきたいこと"
emoji: "🐋"
...
# 目次
1. はじめに
1. Container型仮想化：Dockerとは
1. Container技術関連史
1. Docker一強時代終焉の兆し
1. おわりに

# はじめに
Container型仮想化技術は[Cloud Native技術](https://lethediana.sakura.ne.jp/tech/archives/summary-ja/1893/)における重要なピースであり、すでにIT業界において欠かせない存在になっている。Containerの使用は増加の一途を辿り、今日ではコンテナーベースのマイクロサービスアプリケーションが普及しており[^1-1]、2022年9月のレポートによると、Containerの市場規模は2020年に20.9億米ドル、2028年までに194億米ドルに達すると評価され、2021年から2028年にかけて32.0%のCAGRで成長すると予測されている[^1-2]。

Containerはその高すぎる利便性ゆえ、プログラミングに関連する技術者であれば、何らかの形で一度は触れたことがあるといっても過言ではない。そしてその際、Containerを実行するためのContainer Runtimeとして選ばれてきたソフトウェアが、デファクトスタンダードであるDockerであり、Containerを使うことすなわちDockerを使うことであると認識されていることも少なくない。
しかしながら現在、Docker以外のContainer Runtimeにも目を向けるべき状況になってきている。sysdig社作成の2021 Container Security and Usage Report、Sysdig 2022 Cloud-Native Security and Usage ReportのContainer Platformの利用状況の調査で、2020年に79%だったシェアが2021年に50%まで激減し、さらに2022年にも46%と減少の一途を辿っていると報告された[^1-3] [^1-4]。ただし、このような状況を察知し、Container業界の調査を開始したとしても、Container関連の用語や関係団体が複雑であり状況を紐解くことが難しいのが現状である。
本記事では、Container型仮想化技術にまつわる流れを追い、Docker一強だった時代に終焉が来つつあること、その前に押さえておきたいことについてまとめた。

[^1-1]: [9 Insights on Real-World Container Usage](https://www.datadoghq.com/ja/container-report/)
[^1-2]: [Application Container Market Size And Forecast](https://www.verifiedmarketresearch.com/product/application-container-market/)
[^1-3]: [Sysdig 2021 container security and usage report: Shifting left is not enough](https://sysdig.com/blog/sysdig-2021-container-security-usage-report/)
[^1-4]: [Sysdig 2022 Cloud-Native Security and Usage Report](https://sysdig.com/2022-cloud-native-security-and-usage-report/)
# Container型仮想技術 Docker
## Dockerとは
...
![Dockerk構成](https://lethediana.sakura.ne.jp/tech/wp-content/uploads/2023/03/f527d84598d47468495ba5ea8bd13a41.png "Docker構成")
...
## Container型仮想化のレベル
コンピュータの仮想化は、下記の5つのレベルに分けて考えることができる。なぜ5レベルに分けるのか
...
これを用いることで、OS上で隔離された環境を作ることができるため、実際は一つの物理マシンしか存在しなくても、複数ユーザーがあたかも複数のPCを操作している環境を得たり、アプリケーションをあたかもそれ専用のマシンで実行しているような環境を実現することができる。Hypervisor等を用いたHALレベルの仮想化でも同様のことが可能だが、より軽量な動作ができることが最も大きなメリットといえる（その他のレベルの仮想化については[こちら](https://lethediana.sakura.ne.jp/tech/archives/summary-ja/2040/)をご参考）。
...
# Container技術関連史
本章では、OSレベルの仮想化技術がどのように発展してきたか、それにおけるDockerの立ち位置を追う。
## OSレベルの仮想化技術の立ち上がり（1970~2004年ごろ）
...
## LinuxでのContainer型仮想化の基礎（2004~2012年ごろ）
...
## Containerの普及：Dockerの登場（2012~2015年ごろ）
...
## Container Orchestationの普及：Kubernetesの登場（2014~2018年ごろ）
...
## Docker・Kubernetesの直近の動向（2017年ごろ以降）
...
# Docker一強時代終焉の兆し
Container技術をリーディングしてきたDockerがこれから一切使えなくなるとは考えにくいが、唯一解ではなくなってくる兆しが見えてきている。本章でいくつかの事例を紹介する。[^2-1]
...
## Docker以外のContainer Runtimeの成長
現在、多くのContainer Runtimeがオープンソースで利用可能であり、Containerの実行形式もさまざまである。Dockerが完全に覇権をとったため、いずれもDockerをベンチマークにKubernetesとの連携性、軽量性、セキュリティの高さなど特色も様々である。
